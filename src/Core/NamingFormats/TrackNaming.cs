...
namespace Core.NamingFormats {
    ...
    public class TrackNaming : ITrackNaming {
        ...
        public int FindPrefixLength(IEnumerable<string> trackNames) {
            var titles = trackNames
                .Map(trackName => {
                    var match = _regex.Match(trackName);
                    return match.Groups["title"].Value;
                })
                .ToArray();
            return FindPrefixLengthHelper(titles, 0);
        }

        private int FindPrefixLengthHelper(IReadOnlyList<string> titles, int startIndex) =>
            FindPrefixLengthStartingFrom(titles, startIndex)
                .Match(
                    separatorIndex =>
                        ArePrefixesTheSame(titles, separatorIndex)
                            ? FindPrefixLengthHelper(titles, separatorIndex)
                            : startIndex,
                    () => startIndex
                );

        private Option<int> FindPrefixLengthStartingFrom(IEnumerable<string> titles, int startIndex) {
            const char separator = '-';
            const int separatorLength = 1;

            var separatorIndexes = titles
                .Map(trackTitle => trackTitle.IndexOf(separator, startIndex))
                .ToArray();

            if (!(separatorIndexes.Distinct().Count() == 1 && separatorIndexes[0] > 0)) {
                return None;
            }
        ...
        private bool ArePrefixesTheSame(IEnumerable<string> titles, int startIndex) =>
            titles.Map(title => title[..startIndex]).Distinct().Count() == 1;
